...
**The attributed research corpus for Scruffy.** Edit source-derived observations here. Reconcile operational changes into root `SKILL.md`, its routed references, canonical schemas and regression cases according to [AGENTS.md](../AGENTS.md). There is no compiled principles section in the skill; generated adapters and schema projections have their own canonical owners.

**Operational guard:** these are attributed design leads, not universal prescriptions. Font families, color counts, duplicate icon labels and timing thresholds alone cannot establish a defect, accessibility failure or authorship. Preserve useful redundancy and product-specific conventions; require an observed consequence and the applicable criterion or measured task evidence before admitting a finding. Field observations are maintainer hypotheses with weaker provenance than independently supported research; they remain confirmation-required leads.
...
- **Doherty threshold — the acknowledgment contract.** The historical ~400 ms figure motivates checking acknowledgment — a state change, optimistic UI (§3), skeleton, or honest progress — but is not a universal deadline or an accessibility criterion. Measure the actual interaction and user consequence; use the appropriate current performance metric rather than treating the historical figure as a pass/fail bound. Guards: a fabricated or looping progress indicator is a trust finding, not a remedy (§21); an honest deliberate delay that communicates real work — the source's own perceived-value takeaway — is not a violation when the work is real. [LawsUX][WEBDEV-INP]
