...
When a decisions export is in scope, follow [references/fix-loop.md](references/fix-loop.md): implement only approved open items, author a missing fix packet before changing anything, and never change item status in the current revision. `verification.json` is the required artifact of an implementation run; without it the fix is a claim, not a result.
